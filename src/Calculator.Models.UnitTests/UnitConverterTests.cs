...
namespace CalculatorApp.Model.UnitTests
{
    ...
    public class UnitConverterTests
    {
        ...
        {
            var converter = new StringUnitConverter();
            Assert.IsTrue(ExpectFailure(() => converter.ClaimRatio("b", "a", 0m)));
            Assert.IsTrue(ExpectFailure(() => converter.ClaimRatio("b", "a", 0m, 1m)));
            Assert.IsTrue(ExpectFailure(() => converter.ClaimRatio("a", "a", 1m)));
            converter.ClaimRatio("b", "a", 1m);
            converter.ClaimRatio("d", "c", 1m);
            Assert.IsTrue(ExpectFailure(() => converter.Convert("d", "a", 1m)));
        }
